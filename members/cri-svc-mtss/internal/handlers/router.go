// Package handlers del cri-svc-mtss.
//
// HTTP directos (vía gateway, requieren realm):
//
//	GET /v1/labor/{cedula}/profile
//
...
import (
	...
	"github.com/devsebas/costaricaasservice/members/cri-svc-mtss/internal/service"
)
...
func (a *API) Register(public, app chi.Router) {
	app.Get("/v1/labor/{cedula}/profile", a.laborProfile)

	dispatch := interop.NewDispatcher(a.Registry)
	public.Method("POST", "/_interop/dispatch", dispatch)
}

// RegisterInteropHandlers expone los servicios del MTSS al SS local.
// Otros members (ej. BCR para verificar capacidad de pago de un préstamo)
// pueden consumirlos.
func RegisterInteropHandlers(reg *interop.Registry, svc *service.Service) {
	reg.Register("labor.employment-status", "v1", func(c interop.Context, body []byte) (any, error) {
		var req struct {
		...
		return map[string]any{
			"cedula":      req.Cedula,
			"employments": svc.EmploymentStatusForInterop(c.Realm(), req.Cedula),
		}, nil
	})
}

func (a *API) laborProfile(w http.ResponseWriter, r *http.Request) {
	realm := scrctx.Realm(r.Context())
	cedula := chi.URLParam(r, "cedula")
	out, err := a.Service.LaborProfile(r.Context(), realm, cedula)
	if err != nil {
		httpx.Fail(w, r, err)
		return
	}
