// Package handlers del cri-svc-registro-nacional.
//
// HTTP directos (vía gateway, requieren realm):
//
//	GET /v1/property/{cedula}/profile
//
...
func (a *API) Register(public, app chi.Router) {
	app.Get("/v1/property/{cedula}/profile", a.propertyProfile)

	dispatch := interop.NewDispatcher(a.Registry)
	public.Method("POST", "/_interop/dispatch", dispatch)
}

// RegisterInteropHandlers expone los servicios del Registro Nacional al SS
// local. Otros members (ej. BCR para garantía hipotecaria, Hacienda para
// impuesto solidario) los consumen vía interop firmado.
func RegisterInteropHandlers(reg *interop.Registry, svc *service.Service) {
	reg.Register("property.assets", "v1", func(c interop.Context, body []byte) (any, error) {
		var req struct {
		...
		assets := svc.AssetsForInterop(c.Realm(), req.Cedula)
		assets["cedula"] = req.Cedula
		return assets, nil
	})
}

func (a *API) propertyProfile(w http.ResponseWriter, r *http.Request) {
	realm := scrctx.Realm(r.Context())
	cedula := chi.URLParam(r, "cedula")
	out, err := a.Service.PropertyProfile(r.Context(), realm, cedula)
	if err != nil {
		httpx.Fail(w, r, err)
		return
	}
